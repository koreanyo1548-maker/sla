...
const CharacterLobbyUI={
  ...
  renderModal(campaign){
    ...
    // [2026-09-19] 이 안내는 창을 열면 바로 보이도록 초상화 바로 아래 배너로 낸다 —
    // 능력치·패시브 아래 작은 글씨로 있을 때는 열리는 조건이 눈에 들어오지 않았다.
    ...
    $('#character-modal-content').innerHTML=`<div class="character-modal-scroll"><div class="character-modal-hero">${GameArt.portrait(id)}<div class="character-hero-name"><span class="rarity-chip" style="--rarity-color:${r.color}">${t(r.nameKey)}</span><h2 id="character-modal-title">${t(c.nameKey)}</h2><span class="character-stars">${starMarkup(x.star,CharacterGrowthRules.maxStars)}</span></div></div><div class="character-modal-body">${x.owned?'':`<p class="unlock-banner"><span class="ub-lock" aria-hidden="true">🔒</span><span>${t('character.detail.unlockHint',{module:t(def.labelKey),level:unlockLevel})}</span></p>`}<div class="character-tags"><span>${t(IdentityTable[c.identityId])}</span><span>${t(RaceTable[c.raceId])}</span><span style="color:${def.color}">${t('character.detail.specialty',{module:t(def.labelKey)})}</span></div><div class="character-level-bar"><b>${x.owned?t('common.level',{n:x.level}):t('common.notOwned')}</b><div class="level-track"><span style="width:${x.level/CharacterGrowthRules.maxLevel*100}%"></span></div><span>${CharacterGrowthRules.maxLevel}</span></div>${x.owned?`<small class="meta-note">${t('character.detail.sharedLevel',{module:t(def.labelKey)})}</small>`:''}<div class="character-stats">${[['atk','character.detail.statAtk'],['def','character.detail.statDef'],['hp','character.detail.statHp']].map(([k,labelKey])=>`<div><small>${t(labelKey)}</small><b data-stat="${k}">${I18N.num(stats[k])}</b><small>${canRank?t('character.detail.rankUpStat',{value:rankNext[k]}):t('character.detail.baseStat')}</small></div>`).join('')}</div>${passives.map(pv=>`<div class="character-modal-section character-modal-passive${pv.open?'':' passive-locked'}"><strong>${pv.need?t('character.detail.starPassive',{n:pv.need}):t('character.detail.innatePassive')}</strong><b>${pv.name}${pv.open?'':' 🔒'}</b><ul class="passive-lines">${pv.text.split(' / ').map(line=>`<li>${line}</li>`).join('')}</ul></div>`).join('')}${x.owned?`<div class="shard-line"><span>${t('character.detail.starfire')}</span><b>${I18N.num(starfire)}</b></div>`:''}</div></div><div class="character-modal-footer">${x.owned?`<div class="character-actions">${rankButton()}</div><button class="secondary character-deploy-button" data-character-deploy ${locked||assigned?'disabled':''}>${assigned?t('character.detail.deployed',{module:t(def.labelKey)}):t('character.detail.deploy')}</button>`:`<button class="primary character-deploy-button" data-go-recruit>${t('character.detail.goUpgrade')}</button>`}</div>`;
  },
};
...
const SkillLobbyUI={
  ...
  render(campaign){
    ...
    $('#home-skill-summary').innerHTML=equipped.map(key=>{const m=SKILL_DEFS[key];return `<span class="hs-skill-icon" style="--skill-accent:${m.color}">${GameArt.skill(key)}<em>${t(CONFIG.skills[key].nameKey)}</em></span>`;}).join('');
  ...
  renderModal(campaign,key){
    ...
    // 레벨은 이 창에서 못 올리지만, 슬롯 레벨이 오르면 얼마가 되는지는 여기서 보여줘야
    // 어느 슬롯을 올릴지 정할 수 있다(실제 레벨업은 강화 창에서 한다).
    const nextStats=x.level<CharacterGrowthRules.maxLevel?SkillGrowthSystem.stats(key,{...x,level:x.level+1}):null;
    $('#skill-modal-content').innerHTML=`<div class="character-modal-scroll"><div class="skill-detail-head" style="--skill-accent:${m.color}"><div class="skill-detail-icon">${GameArt.skill(key)}</div><div><h2 id="skill-modal-title">${t(d.nameKey)}</h2><div class="skill-stars">${starMarkup(x.star,CharacterGrowthRules.maxStars)}</div><small>${x.owned?t('skill.detail.levelOfMax',{level:x.level,max:CharacterGrowthRules.maxLevel}):t('common.notOwned')}</small></div></div>${x.owned?'':`<p class="unlock-banner"><span class="ub-lock" aria-hidden="true">🔒</span><span>${unlockTotal?t('skill.detail.unlockHint',{need:unlockTotal,current:LevelTrackSystem.skillLevelTotal(state)}):t('skill.detail.unlockDone')}</span></p>`}<div class="character-modal-section"><strong>${t('skill.detail.active')}</strong><p>${t(m.descKey)}</p><b>${this.effectText(key,x)}</b><small class="skill-detail-meta">${t('skill.detail.cooldown',{sec:SkillGrowthSystem.cooldown(key),growth:d.growth==='duration'?t('skill.detail.growthDuration',{sec:d.durationPerStar}):t('skill.detail.growthEffect')})}</small></div><div class="character-modal-section"><strong>${t('skill.detail.stats')}</strong><div class="character-stats">${[['atk','skill.detail.statAtk'],['def','skill.detail.statDef'],['hp','skill.detail.statHp']].map(([s,labelKey])=>`<div><small>${t(labelKey)}</small><b data-stat="${s}">+${stats[s]}</b><small>${nextStats?t('skill.detail.nextStat',{value:nextStats[s]}):t('skill.detail.maxStat')}</small></div>`).join('')}</div><small class="meta-note">${slot>=0?t('skill.detail.sharedLevel',{n:slot+1}):t('skill.detail.unequippedLevel')}</small></div>${x.owned?`<div class="shard-line"><span>${t('skill.detail.starfire')}</span><b>${I18N.num(starfire)}</b></div>`:''}</div><div class="character-modal-footer">${x.owned?`<div class="skill-detail-actions"><button class="primary" data-skill-action="rankUp" ${!rankAllowed||starfire<rankCost?'disabled':''}>${t('skill.detail.rankUp')}<small>${rankAllowed?t('skill.detail.rankCost',{n:I18N.num(rankCost)}):t('skill.detail.rankMax')}</small></button></div><div class="skill-equip-actions">${Array.from({length:CONFIG.skillPickCount},(_,i)=>i).map(i=>`<button class="secondary" data-skill-slot="${i}">${equipped[i]===key?t('skill.detail.slotEquipped',{n:i+1}):t('skill.detail.slotEquip',{n:i+1})}</button>`).join('')}</div>`:''}</div>`;
