...
const GameArt = {
  ...
  drawCore(ctx,game){
    const p=game.playerPos,r=CONFIG.field.coreRadius;
    const hit=performance.now()<game.coreHitUntil;
    ctx.save();
    ctx.fillStyle='#171C2099';ctx.beginPath();
    ctx.ellipse(p.x,p.y+r*.72,r*.9,r*.28,0,0,Math.PI*2);ctx.fill();
    if(hit){ctx.shadowColor='#F77A3D';ctx.shadowBlur=14;}
    // 미사일 활성화 여부와 관계없이 보호 대상의 크기·불투명도를 유지한다.
    if(!this.drawSprite(ctx,10,p.x,p.y,r*2)){
      ctx.fillStyle=hit?'#F2E9D8':'#5EDBF4';ctx.beginPath();
...
const GameAudio={
  ...
  play(kind='tap'){
    ...
    try{
      ...
      const melody=kind==='reveal'?[392,523,659,784]:kind==='up'?[440,660,880]:[440];
