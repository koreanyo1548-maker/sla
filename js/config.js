...
const DEFAULT_CONFIG = {
  ...
  // [2026-09-18 1차] 중간보스 200 → 100, 시작 에너지 10 → 20으로 조정했다.
  // [2026-09-18 2차] 일반 적 3 → 1점, 중간보스 100 → 140점, 최종보스 0 → 1000점으로 변경했다.
  // [2026-09-18 3차, 확정(인철)] 중간보스를 140 → 70점으로 낮추고, WAVE 시작 시점에도
  // 점수를 지급하는 waveStartBonus를 새로 넣었다 — waveStartBonusFromWave(기본 2)번째
  // WAVE부터 WAVE가 시작될 때마다 waveStartBonus(기본 60)점을 지급한다(WAVE 1은 제외).
  // 시작 에너지(20)와 지급 단위(20점당 1), 생성 비용(1)은 유지한다.
  ...
  stage: {
    ...
    // [2026-09-23] 확정(인철): 76스테이지 균등 맥스 스펙의 10.35배를 적 기준으로 삼는다.
    // HP·공격력 공비 1.117846915는 Lv.1 기준값에서 76스테이지 목표값까지 75번 등비 성장한다.
    hpScaleRatio: 1.117846915,
  ...
  // [2026-09-19] 일괄 합성 — 생성기 왼쪽 버튼 하나로 보드에서 합칠 수 있는 피스를
  // 전부(연쇄 포함) 합친다. 연타로 손 대신 보드를 정리해 버리지 않도록 쿨타임을 둔다.
  batchMerge: {
    cooldownSec: 10,
  },
  ...
  enemy: {
    // 스테이지 1~3은 방어력 0, 스테이지 4부터 10으로 시작해 등비로 증가한다.
    // 1.160091874 공비는 초반 시작값을 유지하면서 76스테이지 방어력 439,999에 도달한다.
    ...
    // [2026-09-23] 확정(인철): 76스테이지 균등 맥스 스펙 × 10.35 기준으로 HP·공격력을 재설정한다.
    base: { hp: 3105, atk: 11385, atkSpeed: 1, travelTimeSec: 10 },
    defenseGrowth:{freeStages:3,base:10,scaleRatio:1.160091874},
    // [2026-09-04] HP 계수와 ATK 계수를 분리했다. 확정(인철): "적의 HP 성장 계수를 늘리면
    // 스킬 원샷·보스 미접근·피격 부족 세 문제가 같이 풀린다".
    // [2026-09-22] 확정(인철): WAVE 후반의 빌드 검사와 피격 위협을 함께 강화한다.
    // HP·ATK가 서로 다른 속도로 자라면 방어력이 성장해도 적의 한 방이 빠르게 무력화되므로,
    // 두 능력치 모두 난이도 단계당 기본값의 35%가 선형 누적되게 맞춘다.
    hpStepPct: 0.35,
    atkStepPct: 0.35,
  ...
  boss: {
    // [2026-09-22] 확정(인철): 중간·최종 보스를 런의 빌드 검사 지점으로 세운다.
    // 공격력 계수는 유지하고 체력만 중간 +25%, 최종 +50%로 올린다.
    midHpExtraMul: 1.25,
    finalHpExtraMul: 1.50,
    midAtkMul: 1.25,
    finalAtkMul: 1.5,
